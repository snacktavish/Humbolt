...
Tree-like reconstructions of species relationships have been applied to species taxonomy since Linnaeus (REF?), but over the past few decades the inference of phylogenetic relationships from DNA sequence data has expanded rabidly. %rabidly, eh?
...
\section*{Aims}
Our aims are to improve on currently existing methods for updating phylogenies. Specifically, to develop a method to add new taxa to trees and rapidly evaluate the likelihood of the new tree that is faster than full re-analysis and more accurate than placement based methods. %I thought placement methods didn't update branches?
 
\section*{Methods}

We have begun developing a pipeline to rapidly and accurately update phylogenetic trees with new taxa. In brief, the pipeline consists of 1) generating a maximum likelihood tree for the starting set of sequences %the startign sequences being the ones before the addition of new taxa?
...
First we perform a full maximum likelihood estimation of the phylogeny for the alignment of the starting set of taxa, using RaxML \cite{stamatakis_RAxML-VI-HPC:_2006}. While in the preliminary pipeline we move forward with only the maximum likelihood tree found in our search, we will test whether using a distribution of trees improves results. %are the panelists who will read this grant the type of people who understand ML? in particular, understanding what an ML estimate is and how it differs from a dist'n of trees?
...
We then calculate the likelihood score for a new tree with this additional branch grafted on at the at location or locations suggested by the BLAST step. By using this new starting tree informed by ancestral state distributions, and building upon previous ML estimates of parameter values, we will be able to rapidly generate trees as good as those estimated from scratch. %as good, again. what does it mean? how will you measure this?

\subsection*{Software}
When we have tested and determined an effective procedure for updating phylogenies with new sequences, we will develop a software package to be used by researches in the field. %open-source?  
...
Ideally the software we develop for rapid phylogenetic updating can be incorporated into open online phylogeny resources, such as The Open Tree of Life project. This project, which Dr. Holder and myself are involved in, is building a synthetic phylogeny incorporating as many species as possible from across the tree of life. %synthetic? as in synthesizing from many sources? I think you might pound on this a little more - this is needed for integration and creating an interface by which publicly-funded research becomes available to the community and to the public.
By integrating rapid phylogeny update tools with this online phylogeny repository we will be able to add taxa to this tree as data are processed, rather than the current lag time of months or even years. % So? who does this help?
...
\section*{Time schedule w/ milestones}
   ...
   (I have no idea what milestones should be....) %I think what you have seems reasonable.
...
% I really like this overall. I'm not sure I know the audience super well. Are they going to know things like ML estimation? I think if you can expect that they will, then you've got a very good proposal here. If they won't, obviously there's some retooling to do. Tried to catch copy edits, but probably missed some.
