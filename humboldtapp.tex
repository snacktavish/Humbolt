...
Tree-like reconstructions of species relationships have been applied to species taxonomy since \mthc{Linnaeus (REF?)}{Lamarck (linnaeus is accurate, but I think that lamarck is the first to use the tree for genealogical relationships, and perhaps that is the better emphasis?)}, but over the past few decades the inference of phylogenetic relationships from DNA sequence data has expanded rabidly. 
Whereas 100 years ago inferring evolutionary relationships required \mthdel{taxonomic expertise and hand} coding of morphological characters across many individual specimens, today millions of DNA base pairs can be sequenced a few hours. 
...
\mth{We have some funding via the NSF-funded Open Tree of Life award (Holder is one of the principal investigators on that award); that would include some funds for me to visit Germany briefly during Dr.~Holder's sabbatical.
...
\mthcomment{Tal Pupko has an algorithm for efficiently sampling the ML joint reconstruction at nodes. We could tweak that to just sample from the posterior at nodes rather than along branches. We might want to mention that briefly.}
...
\mthcomment{Might want to mention that having imputed internal sequences could be a big deal in terms of homology searching. PSI-BLAST tries to exploit phylogenies, but having lots of joing ancestral sequences samples might be an easy way to get better detection of homology in distant comparisons.}
